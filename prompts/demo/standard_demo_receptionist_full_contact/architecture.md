# Standard Demo Receptionist (Email + Phone) - Architecture

This document describes the prompt architecture for the Standard Demo Receptionist assistant with full contact info support (email AND phone).
...
| Contact Policy | Email + Phone (full contact) |

**Purpose:** All-in-one demo receptionist that handles all caller types without handoffs to other agents. Designed for client demos with a simplified, self-contained flow. This variant provides full case manager contact info (both email AND phone) to medical providers and insurance adjusters.
...
<staff_directory>        → Hardcoded staff list with email + phone
...
| medical_provider_flow | Provide case status + email + phone | Get patient name → Search case → Provide info (status + case manager) → Wait for response |
| insurance_adjuster_flow | Provide case status + email + phone | Get claimant name → Search case → Provide info (status + case manager) → Wait for response |
...
4. Look up case_manager_email AND case_manager_phone from staff_directory using staff_id
```
...
### Pattern 6: Contact Info for External Callers (Email + Phone Available)
When medical providers or insurance adjusters ask for case manager contact:
- Only provide what's explicitly requested
- If they ask for phone/number → provide phone only
- If they ask for email → provide email only
- If they ask generically for "contact info" → provide phone first, wait
- If they then ask for email too → provide email
- Phone numbers read as words with natural grouping

**Format for phone numbers (words, not digits):**
- 8708771234 → "eight seven zero, eight seven seven, one two three four"
- NOT "8-7-0, 8-7-7, 1-2-3-4"

This matches the production squad behavior pattern. The key difference from the "only email" variant is that phone numbers ARE available when requested.
...
| **Case Manager Contact** | Email + phone | Phone + email |
| **Staff Directory** | Hardcoded in prompt | RAG-based lookup tool |
| **Firm Config** | Duffy and Duffy | Bey & Associates |

---

## 9. Differences from "only email" Variant

| Aspect | only email | email and phone (this) |
|--------|------------|------------------------|
| Staff directory | email field only | email + phone fields |
...
| Phone format | N/A | Words with grouping |
...
| Name | Staff ID | Role | Email | Phone |
|------|----------|------|-------|-------|
| Brittany | 69 | Case Manager | brittany@pendergastlaw.com | 8708771234 |
| Sarah Johnson | 70 | Case Manager | sarah@pendergastlaw.com | 8708775678 |
...
| Phone numbers as words | "eight seven zero, eight seven seven, one two three four" |
